...
void NOREGEN_5KPH_InitializeDataMapInfo(NOREGEN_5KPH::RT_MODEL_NOREGEN_5KPH_T *
  const NOREGEN_5KPH_M)
{
  // Set C-API version
  rtwCAPI_SetVersion(NOREGEN_5KPH_M->DataMapInfo.mmi, 1);

  // Cache static C-API data into the Real-time Model Data structure
  rtwCAPI_SetStaticMap(NOREGEN_5KPH_M->DataMapInfo.mmi, &mmiStatic);

  // Cache static C-API logging data into the Real-time Model Data structure
  rtwCAPI_SetLoggingStaticMap(NOREGEN_5KPH_M->DataMapInfo.mmi, (nullptr));

  // Set Instance specific path
  rtwCAPI_SetPath(NOREGEN_5KPH_M->DataMapInfo.mmi, (nullptr));
  rtwCAPI_SetFullPath(NOREGEN_5KPH_M->DataMapInfo.mmi, (nullptr));

  // Cache the instance C-API logging pointer
  rtwCAPI_SetInstanceLoggingInfo(NOREGEN_5KPH_M->DataMapInfo.mmi, (nullptr));

  // Set reference to submodels
  rtwCAPI_SetChildMMIArray(NOREGEN_5KPH_M->DataMapInfo.mmi, (nullptr));
  rtwCAPI_SetChildMMIArrayLen(NOREGEN_5KPH_M->DataMapInfo.mmi, 0);
}
